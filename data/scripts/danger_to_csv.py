...
if separate:
    ...
    for f in files:
        ls = []
        with open("../AviDanger/{}".format(f)) as json_file:
    ...
    if include_text:
        data.to_csv("../outputs/ratings_data_text.csv",index=None,header=['Year','Month','Day','Bottom Line','Problem ID','Rank','Likelihood','Location','Size','Discussion'])
    else:
        data.to_csv("../outputs/ratings_data_wo.csv",index=None,header=['Year','Month','Day','Problem ID','Rank','Likelihood','Location','Size'])
    ...
    data.to_csv("../outputs/ratings_comb.csv",index=False,header=['Year','Month','Day','Time','Mountain(s)','ID','Zone ID']+sup_cols)
